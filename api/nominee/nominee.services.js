...
let allQuery = "SELECT * FROM nominee";
let signleQuery = "SELECT * FROM nominee WHERE id=?";
let removeQuery = "DELETE FROM nominee WHERE id=?";


const add__ = async (data) => {
    let {name,dob,user_id,user_type,status}=data;
    let connection = await mysql.createConnection(dbconfig);
    try {
      connection = await mysql.createConnection(dbconfig);
      const [rows, fields] = await connection.execute(addQuery, [name,dob,user_id,user_type,status]);
      return rows;
    } catch (error) {
      return false;
    } finally {
...
const single__ = async (id) => {
  ...
  const update__ = async (data) => {
    let {name,dob,user_id,user_type,status,id}=data;
    let connection = await mysql.createConnection(dbconfig);
    try {
      connection = await mysql.createConnection(dbconfig);
      const [rows, fields] = await connection.execute(updateQuery, [name,dob,user_id,user_type,status,id]);
      return rows;
    } catch (error) {
      return false;
    } finally {
